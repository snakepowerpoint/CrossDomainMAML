...
def train_loop(z_sub, model, model_optim, n_support, n_sub_query, n_iter=25):
  for _ in range(n_iter):
    model.train()
    model.n_query = n_sub_query
    model.n_support = n_support - n_sub_query
    _, model_loss = model.set_forward_loss(z_sub, is_feature=True)
...
def feature_evaluation(cl_data_file, model, model_optim, n_way=5, n_support=5, n_sub_query=1, n_query=15):  # get task
  ...
  z_sub = torch.from_numpy(np.array([z[:n_support] for z in z_all]))
  z_all = torch.from_numpy(np.array(z_all) )
  
  # first update
  model = train_loop(z_sub, model, model_optim, n_support, n_sub_query, n_iter=5)
...
if __name__ == '__main__':
  ...
  # load optimizer
  model_params, ft_params = split_model_parameters(model)
  model_optim = torch.optim.Adam(model_params)
  model_optim.load_state_dict(tmp['model_optim_state'])
  ...
  for i in range(iter_num):
    acc = feature_evaluation(cl_data_file, model, model_optim, n_query=15, **few_shot_params)
    acc_all.append(acc)
